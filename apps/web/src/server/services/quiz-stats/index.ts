...
export async function getQuizStats(missionId: string): Promise<QuizStatsResult> {
    ...
    for (const answer of response.answers) {
      ...
      const isCorrect =
        action.type === ActionType.SHORT_TEXT
          ? gradeShortText(answer.textAnswer, correctOptions, action.matchMode)
          : gradeOptionBased(
...
function gradeOptionBased(selectedIds: string[], correctIds: string[]): boolean {
