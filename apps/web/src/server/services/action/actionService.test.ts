...
describe("ActionService", () => {
  ...
  describe("createScaleAction", () => {
    ...
    it("Mission 소유자가 아니면 403 에러를 던진다", async () => {
      ...
      // When & Then
      await expect(service.createScaleAction(request, "user2")).rejects.toThrow(
        "액션을 추가할 권한이 없습니다.",
      );

      try {
        await service.createScaleAction(request, "user2");
      } catch (error) {
        expect(error instanceof Error && error.cause).toBe(403);
      }
    ...
    it("Mission이 없으면 404 에러를 던진다", async () => {
      ...
      // When & Then
      await expect(service.createScaleAction(request, "user1")).rejects.toThrow(
        "존재하지 않는 미션입니다.",
      );

      try {
        await service.createScaleAction(request, "user1");
      } catch (error) {
        expect(error instanceof Error && error.cause).toBe(404);
      }
  ...
  describe("createSubjectiveAction", () => {
    ...
    it("Mission 소유자가 아니면 403 에러를 던진다", async () => {
      ...
      // When & Then
      await expect(service.createSubjectiveAction(request, "user2")).rejects.toThrow(
        "액션을 추가할 권한이 없습니다.",
      );

      try {
        await service.createSubjectiveAction(request, "user2");
      } catch (error) {
        expect(error instanceof Error && error.cause).toBe(403);
      }
    ...
    it("Mission이 없으면 404 에러를 던진다", async () => {
      ...
      // When & Then
      await expect(service.createSubjectiveAction(request, "user1")).rejects.toThrow(
        "존재하지 않는 미션입니다.",
      );

      try {
        await service.createSubjectiveAction(request, "user1");
      } catch (error) {
        expect(error instanceof Error && error.cause).toBe(404);
      }
  ...
  describe("createEitherOrAction", () => {
    ...
    it("Mission 소유자가 아니면 403 에러를 던진다", async () => {
      ...
      // When & Then
      await expect(service.createEitherOrAction(request, "user2")).rejects.toThrow(
        "액션을 추가할 권한이 없습니다.",
      );

      try {
        await service.createEitherOrAction(request, "user2");
      } catch (error) {
        expect(error instanceof Error && error.cause).toBe(403);
      }
    ...
    it("Mission이 없으면 404 에러를 던진다", async () => {
      ...
      try {
        await service.createEitherOrAction(request, "user1");
      } catch (error) {
        expect(error instanceof Error && error.cause).toBe(404);
      }
