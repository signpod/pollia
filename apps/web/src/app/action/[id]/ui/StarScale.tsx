...
export function MissionStarScale({
  ...
  const { isScaleValueChanged, scaleValue, handleScaleValueChange } = useMissionStarScaleValue(
    actionData.id,
    missionResponse,
    updateCanGoNext,
    onAnswerChange,
...
const DEFAULT_START_SCALE_VALUE = 0;

function useMissionStarScaleValue(
  actionId: string,
  ...
  const initialScaleValue = useMemo(() => {
    if (!missionResponse?.data?.answers || missionResponse.data.answers.length === 0) {
      return DEFAULT_START_SCALE_VALUE;
    }
    ...
    return questionAnswer?.scaleAnswer ?? DEFAULT_START_SCALE_VALUE;
  ...
  useEffect(() => {
    ...
    if (initialScaleValue !== DEFAULT_START_SCALE_VALUE) {
