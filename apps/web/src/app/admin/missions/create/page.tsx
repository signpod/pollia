  ...
  const createMission = useCreateMission({
    onSuccess: async data => {
      ...
      try {
        ...
        toast.error(`${UBIQUITOUS_CONSTANTS.MISSION} 생성 중 오류가 발생했습니다`);
      }
      toast.success(`${UBIQUITOUS_CONSTANTS.MISSION}이 생성되었습니다`);
      router.push(ADMIN_ROUTES.ADMIN_MISSION(data.data.id));
    },
    onError: error => {
      toast.error(error.message || `${UBIQUITOUS_CONSTANTS.MISSION} 생성 중 오류가 발생했습니다`);
    },
  });
    ...
    <div className="px-6 py-8">
      <header className="mb-8">
        <h1 className="text-3xl font-bold tracking-tight">
          새 {UBIQUITOUS_CONSTANTS.MISSION} 만들기
        </h1>
        <p className="text-muted-foreground mt-2">
          {UBIQUITOUS_CONSTANTS.MISSION}의 기본 정보를 입력하세요. 액션과 리워드는 생성 후 추가할 수
          있습니다.
        </p>
      </header>
