...
export function AdminMissionHeader({
  ...
  const duplicateMission = useDuplicateMission({
    onSuccess: data => {
      toast.success(`${UBQUITOUS_CONSTANTS.MISSION}이 복제되었습니다`);
      setIsDuplicateDialogOpen(false);
      router.push(ADMIN_ROUTES.ADMIN_MISSION(data.data.id));
    },
    onError: error => {
      toast.error(error.message || `${UBQUITOUS_CONSTANTS.MISSION} 복제 중 오류가 발생했습니다`);
    },
  });

  const deleteMission = useDeleteMission({
    onSuccess: () => {
      toast.success(`${UBQUITOUS_CONSTANTS.MISSION}이 삭제되었습니다`);
      setIsDeleteDialogOpen(false);
      router.push(ADMIN_ROUTES.ADMIN);
    },
    onError: error => {
      toast.error(error.message || `${UBQUITOUS_CONSTANTS.MISSION} 삭제 중 오류가 발생했습니다`);
    },
  });
  ...
  return (
    <>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <Tooltip>
              ...
              <TooltipContent>{UBQUITOUS_CONSTANTS.MISSION} 복제</TooltipContent>
            </Tooltip>
            ...
            <Tooltip>
              ...
              <TooltipContent>{UBQUITOUS_CONSTANTS.MISSION} 페이지 열기</TooltipContent>
            </Tooltip>
            ...
            <Tooltip>
              ...
              <TooltipContent>{UBQUITOUS_CONSTANTS.MISSION} 삭제</TooltipContent>
            </Tooltip>
          </div>
        </div>
        {nav && <div>{nav}</div>}
      ...
      <AlertDialog open={isDuplicateDialogOpen} onOpenChange={setIsDuplicateDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{UBQUITOUS_CONSTANTS.MISSION}을 복제하시겠습니까?</AlertDialogTitle>
            <AlertDialogDescription>
              {UBQUITOUS_CONSTANTS.MISSION}의 모든 액션과 옵션이 함께 복제됩니다. 복제된{" "}
              {UBQUITOUS_CONSTANTS.MISSION}은 비활성 상태로 생성되며, 제목 끝에 " - 복사본"이
              추가됩니다.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{UBQUITOUS_CONSTANTS.MISSION}을 삭제하시겠습니까?</DialogTitle>
            <DialogDescription>
              이 작업은 되돌릴 수 없습니다. {UBQUITOUS_CONSTANTS.MISSION}과 관련된 모든 데이터가
              삭제됩니다.
              <br />
