...
import { type DateFormInput, dateFormSchema } from "./schemas";
...
export function DateForm({
  ...
  onCancel,
  initialData,
}: ActionFormProps<DateFormData>) {
  const isEditMode = !!initialData;

  const form = useForm<DateFormInput>({
  ...
  const handleSubmit = form.handleSubmit((data: DateFormInput) => {
    onSubmit({
      type: "DATE",
      title: data.title,
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormDescription>
                  사용자가 선택할 수 있는 날짜 개수입니다. 비워두면 1개만 선택 가능합니다.
                </FormDescription>
                <FormMessage />
              </FormItem>
            )}
