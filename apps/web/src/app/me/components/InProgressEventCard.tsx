...
export function InProgressEventCard({ response }: InProgressEventCardProps) {
  ...
  return (
    <Link href={ROUTES.MISSION(mission.id)}>
      <article className="flex w-full items-center gap-4 rounded-2xl border border-zinc-100 bg-white p-4 transition-colors hover:bg-zinc-50">
        {mission.imageUrl ? (
