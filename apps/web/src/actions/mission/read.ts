...
export async function getUserMissions(
  ...
  try {
    ...
    const serverError = new Error(`${UBQUITOUS_CONSTANTS.MISSION} 목록을 불러올 수 없습니다.`);
...
export async function getMission(missionId: string): Promise<GetMissionResponse> {
  try {
    ...
    console.error(`❌ ${UBQUITOUS_CONSTANTS.MISSION} 조회 실패:`, error);
    if (error instanceof Error && error.cause) {
      throw error;
    }
    const serverError = new Error(`${UBQUITOUS_CONSTANTS.MISSION}을 불러올 수 없습니다.`);
    serverError.cause = 500;
    throw serverError;
  }
}
